      ...
      {isExpanded && (<div className="container">
        <div className='section'>
          ...
          <input
            type='date'
            className='input-field'
            onChange={(e) => handleSimpleFieldsChange('afterDate', e.target.value)}
          />
        </div>
        <div className='section'>
          <h3>Clauses</h3>
        ...
        <div className='section'>
          <h3>Deal Type</h3>
          {searchFields.dealTypes.map((dealType, index) => (
            <div key={index} className='input-container'>
              <input
                type='text'
                className='input-field'
                placeholder='Division'
                value={dealType}
                onChange={(e) => handleFieldChange('dealTypes', index, e.target.value)}
              />
              <button className="remove-button" onClick={() => handleFieldRemove('dealTypes', index)}>Remove</button>
            </div>
          ))}
          <button className="add-button" onClick={() => handleFieldAdd('dealTypes')}>Add Another Deal Type</button>
        </div>
